...
Target 1: VOLATILITY (legitimate - volatility clustering exists, ACF ≈ 0.21)
Target 2: RETURN (martingale-consistent - ACF ≈ 0 means unpredictable)
...
def xgboost_ret_walkforward(train_df, test_df):
...
def np_ret_walkforward(train_df, test_df):
    """NeuralProphet for return prediction"""
...
def tft_vol_walkforward(train_df, test_df, bank="UNKNOWN"):
    ...
    return np.array(np.nan_to_num(all_preds, nan=train_df["volatility"].values[-1]))


def tft_ret_walkforward(train_df, test_df, bank="UNKNOWN"):
    """TFT for return prediction"""
    lookback = 24
    train_d = train_df.copy()
    test_d = test_df.copy()
...
# ===== HYBRID (DEVELOPED MODEL: GARCH + Ridge) - VOLATILITY ONLY =====
...
def run_comparison():
    ...
    print("=" * 70)
    print("Target 1: VOLATILITY (legitimate - volatility clustering)")
    print("Target 2: RETURN (martingale-consistent - ACF ~ 0)")
    print("=" * 70)
    ...
    for bank, path in BANK_FILES.items():
        ...
        for fold_idx, (train_pct, val_pct, test_pct) in enumerate(fold_configs):
            ...
            # ===== RETURN PREDICTION (MARTINGALE-CONSISTENT) =====
            # Naive: predict 0 (random walk = no predictable return)
            naive_ret_pred = naive_ret_walkforward(test_df)
            naive_ret_mae = mean_absolute_error(test_ret, naive_ret_pred)

            # XGBoost for return
            xgb_ret_pred = xgboost_ret_walkforward(train_df, test_df)
            xgb_ret_mae = mean_absolute_error(test_ret, xgb_ret_pred)

            # NP for return
            try:
                np_ret_pred = np_ret_walkforward(train_df, test_df)
                np_ret_pred = np_ret_pred[:len(test_ret)]
                np_ret_mae = mean_absolute_error(test_ret, np_ret_pred)
            except Exception as e:
                np_ret_mae = np.nan

            # TFT for return
            try:
                tft_ret_pred = tft_ret_walkforward(train_df, test_df, bank)
                tft_ret_pred = tft_ret_pred[:len(test_ret)]
                tft_ret_mae = mean_absolute_error(test_ret, tft_ret_pred)
            except Exception as e:
                tft_ret_mae = np.nan

            # Hybrid NOT used for return (GARCH predicts volatility, not return)
            hybrid_ret_mae = np.nan

            np_r_str = f"{np_ret_mae:.4f}" if not np.isnan(np_ret_mae) else "ERR"
            tft_r_str = f"{tft_ret_mae:.4f}" if not np.isnan(tft_ret_mae) else "ERR"
            print(f"    RET: Naive={naive_ret_mae:.4f}, XGB={xgb_ret_mae:.4f}, NP={np_r_str}, TFT={tft_r_str}, Hybrid=N/A")
            ...
            fold_results.append({
                ...
                "naive_ret_mae": naive_ret_mae,
                "xgb_ret_mae": xgb_ret_mae,
                "np_ret_mae": np_ret_mae,
                "tft_ret_mae": tft_ret_mae,
                "hybrid_ret_mae": hybrid_ret_mae,
            })
        ...
        if fold_results:
            ...
            print(f"    RETURN:")
            print(f"      Naive:   {avg['naive_ret_mae']:.6f}")
            print(f"      XGBoost: {avg['xgb_ret_mae']:.6f}")
            print(f"      NP:      {avg['np_ret_mae']:.6f}")
            print(f"      TFT:     {avg['tft_ret_mae']:.6f}")

            df_folds.to_csv(OUTPUT_DIR / f"{bank}_4fold_vol_ret.csv", index=False)
            ...
            ret_models = {
                "Naive": avg["naive_ret_mae"],
                "XGBoost": avg["xgb_ret_mae"],
                "NP": avg["np_ret_mae"],
                "TFT": avg["tft_ret_mae"],
            }
            best_ret = min(ret_models, key=ret_models.get)
            ...
            all_results.append({
                ...
                "avg_naive_ret": avg["naive_ret_mae"],
                "avg_xgb_ret": avg["xgb_ret_mae"],
                "avg_np_ret": avg["np_ret_mae"],
                "avg_tft_ret": avg["tft_ret_mae"],
                "best_vol": best_vol,
                "best_ret": best_ret,
            })

    df_summary = pd.DataFrame(all_results)
    df_summary.to_csv(OUTPUT_DIR / "4fold_vol_ret_summary.csv", index=False)
    ...
    ax1.set_ylabel("MAE")
    ax1.set_title("Volatility Prediction: 5 Models (Legitimate)")
    ...
    ax2 = axes[1]
    ret_maes = {
        "Naive": df_summary["avg_naive_ret"].values,
        "XGB": df_summary["avg_xgb_ret"].values,
        "NP": df_summary["avg_np_ret"].values,
        "TFT": df_summary["avg_tft_ret"].values,
    }

    for i, (model, color) in enumerate(zip(["Naive", "XGB", "NP", "TFT"], colors[:4])):
        ax2.bar(x + i*width, ret_maes[model], width, label=model, color=color)
    ax2.set_ylabel("MAE")
    ax2.set_title("Return Prediction: 4 Models (Martingale-Consistent)")
    ax2.set_xticks(x + width * 1.5)
    ax2.set_xticklabels(banks)
    ax2.legend()

    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "4fold_vol_ret_comparison.png", dpi=150)
    plt.close()

    print("\n" + "=" * 70)
    print("SUMMARY: VOLATILITY & RETURN PREDICTION")
    print("=" * 70)
    for _, r in df_summary.iterrows():
        ...
        print(f"  RETURN: Naive={r['avg_naive_ret']:.4f}, XGB={r['avg_xgb_ret']:.4f}, NP={r['avg_np_ret']:.4f}, TFT={r['avg_tft_ret']:.4f}")
        print(f"  BEST VOLATILITY: {r['best_vol']}, BEST RETURN: {r['best_ret']}")
